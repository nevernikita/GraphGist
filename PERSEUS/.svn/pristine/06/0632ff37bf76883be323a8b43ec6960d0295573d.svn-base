/*******************************************************************************
VecarrvecCache.java - Naive matrix-vector multiplication in hadoop.
...
Description: VecarrvecCache is a hadoop program which performs matrix-vector multiplication using element-wise join.
...
public class VecarrvecCache extends Configured implements Tool 
{
    //////////////////////////////////////////////////////////////////////
	...
	public static class MapPass1 extends MapReduceBase implements Mapper<LongWritable, Text, IntWritable, DoubleWritable>
    {
		...
		public void configure(JobConf job) {
			String vecarr_path_prefix = job.get("vecarr_path_prefix");
			String input_file = job.get("map.input.file");
			String right_vec_str = job.get("right_vector");

			int lzq_index = input_file.indexOf(vecarr_path_prefix);
			...
			System.out.println("VecarrvecCache.MapPass1: makesym=" + makesym + ", right_vector=" + right_vec_str + ", col=" + col);
    ...
    public static class RedPass1 extends MapReduceBase implements Reducer<IntWritable, Text, Text, DoubleWritable>
    {
		...
		public void configure(JobConf job) {
			...
			System.out.println("VecarrvecCache.RedPass1: makesym=" + makesym + ", right_vector=" + right_vec_str);
	...
	public static class MapPass2 extends MapReduceBase implements Mapper<LongWritable, Text, Text, DoubleWritable>
    {
		public void map (final LongWritable key, final Text value, final OutputCollector<Text, DoubleWritable> output, final Reporter reporter) throws IOException
		{
			...
			output.collect( new Text(line[0]), new DoubleWritable( Double.parseDouble(line[1]) ) );
		}
	}

    public static class RedPass2 extends MapReduceBase implements Reducer<Text, DoubleWritable, Text, DoubleWritable>
    {
    ...
    {
		final int result = ToolRunner.run(new Configuration(), new VecarrvecCache(), args);
    ...
    {
		System.out.println("VecarrvecCache <left edge_path> <# of reducers> <right edge file> <m> <n>");
    ...
    public int run (final String[] args) throws Exception
    {
		...
		path_prefix = args[0];
		lz_q_no = Integer.parseInt(args[1]);//edge_path = new Path(args[0]);
		nreducers = Integer.parseInt(args[2]);
		String right_vec_file = args[3];
		int right_vec_length = Integer.parseInt(args[4]);
		...
		System.out.println("Running VecarrvecCache...");
		JobClient.runJob( configPass1(right_vec_str) );
		//JobClient.runJob(configPass2());
		System.out.println("VecarrvecCache Done. Output is saved in HDFS " + output_path.getName() );
    ...
    {
		final JobConf conf = new JobConf(getConf(), VecarrvecCache.class);
		conf.set("right_vector", "" + in_right_vec_str);
		conf.set("vecarr_path_prefix", path_prefix);
		conf.setJobName("VecarrvecCache_pass1_" + job_name_base);
		...
		FileInputFormat.setInputPaths(conf, path_prefix + 1);  //FileInputFormat.setInputPaths(conf, "lz_q1");  
		for(int i = 2; i <= lz_q_no; i++) 
			FileInputFormat.addInputPaths(conf, (path_prefix + i));  
