...
public class RWRNaive extends Configured implements Tool 
{
	...
	protected Path minmax_path = null; //= new Path("rwr_minmax");
	protected Path distr_path = null; // = new Path("rwr_distr");
...
//		vector_path = new Path("rwr_vector");
		...
		vector_path = new Path(args[8]);
		minmax_path = new Path(args[9]);
		distr_path = new Path(args[10]);
