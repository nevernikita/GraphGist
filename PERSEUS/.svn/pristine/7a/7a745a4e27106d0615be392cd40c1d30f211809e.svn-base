...
import pegasus.PagerankNaive.MapStage2b;
...
public class PagerankBlock extends Configured implements Tool 
{
	...
	//  - Output: (node_id, "v"PageRank_of_the_node)  -- danai: removed "v" from output
    //////////////////////////////////////////////////////////////////////
    public static class	MapStage25 extends MapReduceBase implements Mapper<LongWritable, Text, IntWritable, Text>
    {
		...
		public void map (final LongWritable key, final Text value, final OutputCollector<IntWritable, Text> output, final Reporter reporter) throws IOException
		{
			...
			for(i = 0; i < tokens.length; i+=2) {
				...
				/* danai: changed output format - removed "v" */
//				output.collect( new IntWritable(block_width * block_id + elem_row), new Text("v" + pagerank) );
				output.collect( new IntWritable(block_width * block_id + elem_row), new Text( Double.toString(pagerank) ) );
			}
		}
    }

//////////////////////////////////////////////////////////////////////
    // PASS 2.5: process the converged block PageRank results to plain format.
	//         This is a map-only stage.
	//  - Input: (node_id, "v"PageRank_of_the_node)
	//  - Output: (node_id, PageRank_of_the_node)  -- danai: removed "v" from output
    //////////////////////////////////////////////////////////////////////
    public static class	MapStage2b extends MapReduceBase implements Mapper<LongWritable, Text, IntWritable, DoubleWritable>
    {
		int block_width;
		...
		public void configure(JobConf job) {
			...
			System.out.println("MapStage2b: block_width = " + block_width);
		}
		...
		{
			final String[] line = value.toString().split("\t");
			final String prField = line[1].toString().substring(1);

			output.collect(new IntWritable(Integer.parseInt(line[0])), new DoubleWritable(Double.parseDouble(prField)) );
		}
    }
    ...
    protected Path vector_unfold_path = null; //new Path("prblk_vector");
	protected Path minmax_path = null; //new Path("prblk_minmax");
	protected Path distr_path = null; //new Path("prblk_distr");
...
//		System.exit(result);
    ...
    {
		System.out.println("PagerankBlock <edge_path> <vector_path> <tempmv_path> <output_path> <# of nodes> <# of reducers> <number of iterations> <block width>");
    ...
    public int run (final String[] args) throws Exception
    {
		...
		block_width = Integer.parseInt(args[7]);
		vector_unfold_path = new Path(args[8]); //new Path("prblk_vector");
		minmax_path = new Path(args[9]); //new Path("prblk_minmax");
		distr_path = new Path(args[10]); //new Path("prblk_distr");
		...
		fs.copyToLocalFile(vector_unfold_path, new Path(vector_unfold_path+"_local"));
		fs.copyToLocalFile(minmax_path, new Path(minmax_path+"_local"));
		fs.copyToLocalFile(distr_path, new Path(distr_path+"_local"));
		
		System.out.println("\n[PEGASUS] PageRank computed.");
		System.out.println("[PEGASUS] The final PageRanks are in the HDFS prblk_vector.");
		System.out.println("[PEGASUS] The minium and maximum PageRanks are in the HDFS prblk_minmax.");
    ...
    // Configure pass2b: by danai (removing 'v' from pagerank output)
    protected JobConf configStage2b() throws Exception
    {
		final JobConf conf = new JobConf(getConf(), ConCmptBlock.class);
		conf.set("block_width", "" + block_width);
		conf.setJobName("Pagerank_Stage2b");

		conf.setMapperClass(MapStage2b.class);
