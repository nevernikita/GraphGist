...
- ParseCT is an eclipse project. all it does is read the ClinicalTrials.xml from resources and output ConditionInvestigator.csv
- ct_db_creator creates the sqlite3 database from ConditionInvestigator.csv (from command run "./csv2db.bash ../ConditionInvestigator.csv")
- FocusG is Namit's demo as an eclipse project, with external jars in their own folder (relative path)
