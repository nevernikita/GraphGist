/*******************************************************************************
MatmatCache.java - Naive matrix-vector multiplication in hadoop.
...
Description: MatmatCache is a hadoop program which performs matrix-vector multiplication using element-wise join.
...
public class MatmatCache extends Configured implements Tool 
{
    //////////////////////////////////////////////////////////////////////
	...
	public static class MapPass1 extends MapReduceBase implements Mapper<LongWritable, Text, Text, DoubleWritable>
    {
		...
		// read right matrix from string encoding.
		// The format of the string is #_ROW  #_COL   elem(1,1)  elem(1,2) ... elem(#_ROW, #_COL)
		...
		public void configure(JobConf job) {
			...
			System.out.println("MatmatCache.MapPass1: makesym=" + makesym + ", right_matrix=" + right_mat_str);
		...
		public void map (final LongWritable key, final Text value, final OutputCollector<Text, DoubleWritable> output, final Reporter reporter) throws IOException
		{
			...
			if( line[2].charAt(0) == 'v' )
				cur_val = Double.parseDouble(line[2].substring(1));
			else
				cur_val = Double.parseDouble(line[2]);
			...
			for(int i = 0; i < rm_col; i++) {
				...
				double out_val = cur_val * RM.get(dst_id, i);
    ...
    {
		final int result = ToolRunner.run(new Configuration(), new MatmatCache(), args);
    ...
    {
		System.out.println("MatmatCache <left edge_path> <# of reducers> <right edge file> <m> <n>");
    ...
    public int run (final String[] args) throws Exception
    {
		...
		System.out.println("Running MatmatCache...");
		JobClient.runJob( configPass1(edge_path, right_matrix_str) );
//		JobClient.runJob(configPass2());
		System.out.println("MatmatCache Done.");

		return 0;
    }

	// Configure pass1
    protected JobConf configPass1 (Path edge_path, String in_right_matrix_str) throws Exception
    {
		final JobConf conf = new JobConf(getConf(), MatmatCache.class);
