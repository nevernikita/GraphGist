...
import pegasus.Hadi.MapStage4b;
...
//					output.collect( new IntWritable(block_width * block_id + elem_row), new Text( max_radius + "\t" + df.format(eff_radius) ) );
...
//		System.exit(result);
    ...
    {
		System.out.println("hadiblock <edge_path> <curbm_path> <tempbm_path> <nextbm_path> <output_path> <# of nodes> <# of replication> <# of reducers> <enc or noenc> <newbm or contNN> <block_width> <output_folder> <output_summary_folder>");
    ...
    public int run (final String[] args) throws Exception
    {
		...
		tmp_path = new Path(output_path + "_temp");
		number_nodes = Integer.parseInt(args[5]);
		radius_path = new Path(args[12]); //new Path("hadi_radius_block");
		radius_summary_path = new Path(args[13]); //new Path("hadi_radius_block_summary");
		...
		// danai: changed the output value from (bsf:max:effective)
		// to (max \t effective)
