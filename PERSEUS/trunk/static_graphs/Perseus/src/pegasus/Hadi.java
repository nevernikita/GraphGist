...
//				output.collect( new IntWritable(Integer.parseInt(line[0])), new Text( max_radius + "\t" + eff_radius) );
	...
	//  - Output: (node_id, "bsf"max_radius:eff_radius)
    //////////////////////////////////////////////////////////////////////
    public static class	MapStage4b extends MapReduceBase implements Mapper<LongWritable, Text, IntWritable, Text>
    {
		// input sample :
		// 0       bsf:10:8
		public void map (final LongWritable key, final Text value, final OutputCollector<IntWritable, Text> output, final Reporter reporter) throws IOException
		{
			final String[] line = value.toString().split("\t");
			final String[] tokens = line[1].split(":");
			String max_radius = tokens[0].substring(3); // e.g., bsf5
			String eff_radius = tokens[1];
...
//		System.exit(result);
    ...
    {
		System.out.println("hadi <edge_path> <curbm_path> <tempbm_path> <nextbm_path> <output_path> <# of vertices> <# of replication> <# of reducers> <enc or noenc> <newbm or contNN> <makesym or reg> <'max' or maximum_iteration> <output_folder> <output_summary_folder>");
...
//		radius_summary_path = new Path("hadi_radius_summary");
		...
		// danai: splitting the value field to max and effective radius 
		System.out.println("Changing the output format...");
		JobClient.runJob(configStage4b());
		...
		if( i > max_iteration ) 
			System.out.println("Reached Max Iteration " + max_iteration);
		...
		fs.delete(radius_path, true);
		fs.rename(tmp_path, radius_path);
		fs.copyToLocalFile(radius_path, new Path(radius_path+"_local"));
		fs.copyToLocalFile(radius_summary_path, new Path(radius_summary_path+"_local"));
 ...
 // Configure Stage4b -- danai splitting the max and effective radius of the output
    // up to this point it is of the form bsf:max:effective
    protected JobConf configStage4b () throws Exception
    {
		final JobConf conf = new JobConf(getConf(), Hadi.class);
		conf.setJobName("HADI_Stage4b");
		
		conf.setMapperClass(MapStage4b.class); 
