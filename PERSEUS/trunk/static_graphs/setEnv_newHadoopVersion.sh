...
if [[ "$OS" == "$MAC" ]]; then
   ...
   # Editing DEMO/hadoop-2.3.0/conf/hadoop-env.sh
   sed "s#.*export JAVA_HOME=.*#export JAVA_HOME=$javaHomeFirst#" hadoop-2.3.0/etc/hadoop/hadoop-env.sh > tmp.txt
   mv tmp.txt hadoop-2.3.0/etc/hadoop/hadoop-env.sh

   # Editing DEMO/hadoop-2.3.0/bin/hadoop
   sed "s#.*JAVA=\$JAVA_HOME/bin/java.*#JAVA_HOME=\$JAVA_HOME/$javaHomeSecond#" hadoop-2.3.0/bin/hadoop > tmp.txt
   mv tmp.txt hadoop-2.3.0/bin/hadoop
######################################################################################
elif [[ "$OS" == "$LINUX" ]] ; then
   # linux
   echo $OS
...
echo "export HADOOP_HOME=$currentDir/hadoop-2.3.0" >> $homeDir/.bashrc
echo "export PATH=\${HADOOP_HOME}/bin:\${PATH}" >> $homeDir/.bashrc
echo "export HADOOP_VERSION=2.3.0" >> $homeDir/.bashrc
